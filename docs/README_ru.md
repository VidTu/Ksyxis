...
4. Выполните в терминале/PowerShell команду `./gradlew assemble`
...
- `ru.vidtu.ksyxis.debug.asserts`: Оставить декларации `assert` после
...
- `ru.vidtu.ksyxis.debug.jars`: Создать дополнительные два JAR-ника с
  Javadoc-ами и исходниками. Приведёт к разделению папки `build/libs`.

Более точечные опции имеют приоритет над глобальной.
Все опции отладки *отключены* по умолчанию. Смотрите
[документацию Gradle](https://docs.gradle.org/current/userguide/build_environment.html#sec:project_properties)
...
можете установить логическую опцию Gradle `ru.vidtu.ksyxis.slim` на `true`.

Эта опция несовместима с опциями "Отладочных JAR-ников".

###### Чекни [SECURITY.md](SECURITY.md), если ты инсекьюр. <3
